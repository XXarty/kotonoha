...
**首发平台：** Web（桌面与移动端响应式）  
**部署目标：** Vercel  
**数据与身份：** Neon PostgreSQL + Better Auth
...
- Better Auth 管理用户、会话和登录状态，认证表与会话持久化在同一个 Neon PostgreSQL 数据库。
...
Better Auth 的用户、会话、账户和验证表通过 Drizzle 迁移保存在同一个 Neon PostgreSQL 数据库。学习进度、收藏等业务表只保存 Better Auth 用户 ID，不保存密码。
...
- Better Auth 负责邮箱密码登录和服务端会话校验，认证数据持久化到 Neon PostgreSQL。
...
1. 完成内容导入、测试和生产构建。
2. 初始化 Neon PostgreSQL 项目、Better Auth 认证表和生产数据库迁移。
3. 将数据库连接与 Better Auth 密钥/地址配置写入 Vercel 环境变量。
